...
{
    zc_assert(a_level, );
    zc_profile(flag, "---level[%p][%d,%s,%s,%d,%d]---", a_level, a_level->int_level,
               a_level->str_uppercase, a_level->str_lowercase, (int)a_level->str_len,
...
zlog_level_t *zlog_level_new(char *line)
{
    ...
    snprintf(a_level->color_str_upper, sizeof(a_level->color_str_upper), "%s%s\033[0m", color,
             a_level->str_uppercase);
    snprintf(a_level->color_str_lower, sizeof(a_level->color_str_lower), "%s%s\033[0m", color,
