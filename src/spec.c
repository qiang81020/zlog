...
{
    zc_assert(a_spec, );
    zc_profile(flag, "----spec[%p][%.*s][%s|%d][%s,%ld,%ld,%s][%s]----", a_spec, a_spec->len,
               a_spec->str, a_spec->time_fmt, a_spec->time_cache_index, a_spec->print_fmt,
               (long)a_spec->max_width, (long)a_spec->min_width,
               a_spec->left_fill_zeros ? "true" : "false", a_spec->mdc_key);
    return;
}
...
static int zlog_spec_write_time_internal(zlog_spec_t *a_spec, zlog_thread_t *a_thread,
                                         zlog_buf_t *a_buf, short use_utc)
{
...
static int zlog_spec_write_time_local(zlog_spec_t *a_spec, zlog_thread_t *a_thread,
                                      zlog_buf_t *a_buf)
{
...
{
    return zlog_buf_append(a_buf, a_thread->event->category_name,
...
static int zlog_spec_write_level_lowercase(zlog_spec_t *a_spec, zlog_thread_t *a_thread,
                                           zlog_buf_t *a_buf)
{
    ...
    return zlog_buf_append(a_buf, a_level->color_str_lower, strlen(a_level->color_str_lower));
}

static int zlog_spec_write_level_uppercase(zlog_spec_t *a_spec, zlog_thread_t *a_thread,
                                           zlog_buf_t *a_buf)
{
    zlog_level_t *a_level;
    ...
    return zlog_buf_append(a_buf, a_level->color_str_upper, strlen(a_level->color_str_upper));
}
...
static int zlog_spec_write_usrmsg(zlog_spec_t *a_spec, zlog_thread_t *a_thread, zlog_buf_t *a_buf)
{
    if (a_thread->event->generate_cmd == ZLOG_FMT)
    {
        ...
        {
            return zlog_buf_vprintf(a_buf, a_thread->event->str_format, a_thread->event->str_args);
        }
        else
        {
    ...
    else if (a_thread->event->generate_cmd == ZLOG_HEX)
    {
        ...
        while (1)
        {
            ...
            for (byte_offset = 0; byte_offset < 16; byte_offset++)
            {
                ...
                {
                    c = *((unsigned char *)a_thread->event->hex_buf + line_offset * 16 +
                          byte_offset);
...
static int zlog_spec_gen_msg_reformat(zlog_spec_t *a_spec, zlog_thread_t *a_thread)
{
    ...
    return zlog_buf_adjust_append(a_thread->msg_buf, zlog_buf_str(a_thread->pre_msg_buf),
                                  zlog_buf_len(a_thread->pre_msg_buf), a_spec->left_adjust,
                                  a_spec->left_fill_zeros, a_spec->min_width, a_spec->max_width);
}
...
static int zlog_spec_gen_path_reformat(zlog_spec_t *a_spec, zlog_thread_t *a_thread)
{
    ...
    return zlog_buf_adjust_append(a_thread->path_buf, zlog_buf_str(a_thread->pre_path_buf),
                                  zlog_buf_len(a_thread->pre_path_buf), a_spec->left_adjust,
                                  a_spec->left_fill_zeros, a_spec->min_width, a_spec->max_width);
}
...
static int zlog_spec_gen_archive_path_reformat(zlog_spec_t *a_spec, zlog_thread_t *a_thread)
{
    ...
    return zlog_buf_adjust_append(a_thread->archive_path_buf, zlog_buf_str(a_thread->pre_path_buf),
                                  zlog_buf_len(a_thread->pre_path_buf), a_spec->left_adjust,
                                  a_spec->left_fill_zeros, a_spec->min_width, a_spec->max_width);
}
